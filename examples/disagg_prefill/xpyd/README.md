...
For xPyD multi-round QA scenario, proxy can provide the clients with bounded prefiller/decoder for the same session,
other than round-robin.

http requests should provide a header (e.g., 'session-id: \<unique-id\>') for the proxy to bind the prefiller/decoder for the duration of a session:
...
- **Header Name**: The name of the header field (e.g., 'session-id') for http proxy to find the \<unique-id\>. \
